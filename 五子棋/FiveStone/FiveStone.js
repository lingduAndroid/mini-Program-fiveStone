...
export default class FiveStone {
    ...
    constructor(chessBoardSize, chessboardSizePercent) {
        ...
        for (var r = 0; r < chessBoardSize; r++) {
            ...
            for (var c = 0; c < chessBoardSize; c++) {
                row.push({
                    stoneType:Stone.none,  //正在下棋的是1号还是2号，0是无，1是黑，2是白
                    //位置使用到的时候才计算
                    // pos:null
                });
        ...
        wx.getSystemInfo({
            success: function(res) {
              //设置棋盘宽度属性chessboardSizePX
                ...
                self.cellSizePX = self.chessboardSizePX / (chessBoardSize - 1); //每一个格子的宽度，格子是能放置的数减1
                console.log('%c单元格大小:%c' +
                 self.cellSizePX +
                  ...
                  self.halfCellSizePX = self.cellSizePX * 0.5;  //半格
            }
        });
        //当前下子的类型
    ...
    getStepLocation(e) {
        ...
        var stepPos = {
            x: Math.ceil((clientPos.x - this.halfCellSizePX) / this.cellSizePX),   //返回大于等于最小整数，相对棋盘位置除去单元格宽度为棋盘的位置
    ...
    getStepPosition(e) {
        ...
        var absPos = {
          x: stepPos.x * this.cellSizePX + curTarget.offsetLeft - this.halfCellSizePX - 1,   //在界面上的xy位置
          y: stepPos.y * this.cellSizePX + curTarget.offsetTop - this.halfCellSizePX - 1
        };
    ...
    step(x, y) {
        if (this.canStepAt(x, y)) {
            ...
            this.stone = nowStone == Stone.black ? Stone.white:Stone.black;  //为黑stone为white,为白为空就为black
    ...
    undo() {
        ...
        this.history.splice(lastStoneIndex, 1);          //删除上次落子记录
        this.chessBoard[lastStone.x][lastStone.y].stoneType = Stone.none; //棋子位置的数组中上次落子位子设为空
        this.allowStep();   //允许落子
    }
